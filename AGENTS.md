...
- **SCIP extraction — three precision improvements (0.1.34)**: (1) `end_line` is now populated from `occ.enclosing_range[2]+1` on function/class definition occurrences (was always equal to `line_number` before — zero-range nodes). (2) `Class→Function CONTAINS` edges are emitted from `method_class_pairs` collected during Pass 1 (method SCIP symbols carry `ClassName#method().`; the class sym is `ClassName#`). These edges use the existing `_EDGE_TABLES["CONTAINS"] = [("Class", "Function")]` pair. (3) Star-import call fallback in Pass 2: when `uid_map.get(sym)` returns None AND `sym.endswith("/")` (SCIP resolved the call to the module namespace instead of the specific function — Pyright limitation with `from mod import *`), extract the call token from `src[row][col_s:col_e]` and look it up in `name_path_index`, a `(short_name, abs_path) → uid` dict built during Pass 1 for Function nodes only. On `~/backend-tss-api_v2`: CALLS 2,760→3,469 (+26%), IMPORTS 738→1,030 (+40%), CONTAINS 0→886, INHERITS 37→63, and 1,891/1,893 functions now have a real body range.
- **TS/JS monorepo support (`extract/scip.py::_find_ts_packages`)**: scip-typescript invoked at a workspace root with a project-references aggregator `tsconfig.json` produces zero documents — it doesn't traverse `packages/*/tsconfig.json`. `ScipExtractor.run()` detects sub-packages under conventional workspace parents (`_TS_PKG_PARENTS = {packages, apps, services, libs, modules}`) and runs `_run_per_package`, which spawns scip-typescript once per sub-package, parses each scip with `doc_root=pkg_dir` so document paths resolve relative to the sub-package, merges by deduping nodes on uid and edges on tuple key, and logs `"indexed N/M sub-package(s) → X nodes, Y edges"` to stderr. Without this, every TS monorepo (Next.js, NestJS, Remix, Turborepo) extracted 0 nodes — verified on `tinyhttp` (0 → 261 nodes after enabling).
- **Cross-package IMPORTS via global `sym_def_path` + `paths` injection (`extract/scip.py::_collect_global_sym_def_paths`, `_augmented_tsconfig`, `_workspace_pkg_sources`)**: closing the cross-package IMPORTS gap needs three cooperating pieces. (1) **`paths` injection** — scip-typescript invoked in a workspace sub-package can't resolve `@scope/pkg` imports to source on its own: `node_modules/@scope/pkg` typically points to a `dist/` that may not exist (unbuilt) or that scip-typescript can't follow (pnpm symlinks + un-flattened layout). `_workspace_pkg_sources` reads each sub-package's `package.json` `name` and locates its source-entry (`src/index.ts` etc.); `_augmented_tsconfig` is a context manager that *temporarily* writes those into `compilerOptions.paths` (merging without clobbering existing aliases), then restores the original on exit. (2) **Global `sym_def_path` pre-sweep** — each per-package scip parsed in isolation only knows its own definitions, so even with cross-package refs emitted, Pass 4 can't resolve them. `_collect_global_sym_def_paths` walks every per-package scip once for definition occurrences and builds a single `symbol → abs path` map, plumbed into each `_parse_scip` call via `sym_def_path_global`. (3) **Canonical paths** — `_abs_doc_path` collapses `..` via `os.path.normpath` so the same file always has the same graph key whether emitted by package a or package b's scip. Together: tinyhttp `orgraph deps packages/app/src/app.ts` went from 2 → 8 deps post-fix, including cross-package edges to `@tinyhttp/router`, `@tinyhttp/req`, `@tinyhttp/url`. Same-package and external imports continue to work as before.
- **TS arrow-function exports (`extract/scip.py::_ARROW_FN_DOC_RE`, `_label_from_symbol_with_doc`)**: `export const fn = () => {}` comes through SCIP with a `.` (term) descriptor — same suffix as plain `const PI = 3.14`. Labeling every `.`-suffixed symbol Function would pollute the graph with every constant and ESM re-export. Discriminator: pre-scan `SymbolInformation.documentation` for `=>` (matched by `_ARROW_FN_DOC_RE` with negative lookbehinds to skip `==>`, `<=>`, `!=>`, `>=>`), build an `arrow_fn_syms` set, then `_label_from_symbol_with_doc(symbol, arrow_fn_syms)` returns `"Function"` only when both the descriptor ends in `.` and the symbol is in the set. Plain values like `export const VERSION = "1.0.0"` (whose hover is `var VERSION: "1.0.0"` — no `=>`) correctly skip. Pass 2 (call edges) needs no changes — once the arrow-fn is in `label_of` as Function, its callsites resolve. Verified on tinyhttp: `who-calls pushMiddleware` returns `Router.all`, `Router.use` instead of "Symbol not found."
- **Go multi-module / `go.work` support (`extract/scip.py::_find_go_modules`)**: parallel to `_find_ts_packages` for TS. scip-go invoked at the root of a multi-module repo only indexes one module — the others are invisible. `_find_go_modules` walks `rglob("go.mod")` (modules can live anywhere — `services/auth/`, `cmd/server/`, `grpc/example1/`, not restricted to a `packages/` parent), and `_run_per_package` (now `(pkgs, binary, lang)`) is language-aware: builds the scip command via `_build_command` (scip-go is `scip-go --output ...`, no `index` subcommand), and only runs `_augmented_tsconfig` for scip-typescript. Verified on `gin-gonic/examples`: 83 → 159 nodes, 8 → 18 HTTP handlers, including handlers from `realtime-chat/` and `websocket/server/` sub-modules.
- **scip-go cross-module symbol normalisation (`extract/scip.py::_scip_unversioned`, `_SCIP_VERSION_SEG_RE`)**: scip-go emits the same symbol with different version segments depending on which module's scip you read. From module A's perspective, importing `example.com/b` emits a reference with version `.` (placeholder); module B's own definition uses the resolved module hash. Same logical symbol, different string. `_collect_global_sym_def_paths` stores BOTH the original symbol and an unversioned form (version segment replaced with `.`); Pass 4's lookup falls back to the unversioned form when an exact match misses. Without this, every cross-module Go IMPORTS edge dropped. Same normalisation pattern works for scip-typescript and scip-python versioning.
- **Go `import ( ... )` block IMPORTS (`extract/scip.py::_IMPORT_LINE_RE`)**: the regex was extended with a fourth alternation matching a bare indented quoted module path (optionally preceded by a named-import alias like `pkg`, `_`, `.`): `^\s*(?:[A-Za-z_]\w*\s+|\.\s+|_\s+)?["\\`][^"\\`]+["\\`]\s*$`. Matches `\t"net/http"` and `\tpkg "github.com/foo/bar"` etc. End-anchored so it doesn't accidentally match string literals appearing mid-expression elsewhere.
- **FastAPI / Starlette / APIRouter / Flask HTTP entry-point heuristics (`extract/treesitter.py::_collect_python_http_routes`)**: a single Python collector handles two decorator shapes. FastAPI/Starlette/APIRouter use verb-named decorators (`@app.get("/x")`, `@router.post("/y")`) — `_FASTAPI_ROUTE_RE` captures the verb and path. Flask uses `@app.route("/x", methods=["POST"])` — `_FLASK_ROUTE_RE` + `_FLASK_METHODS_KW_RE` + `_FLASK_VERB_LITERAL_RE` extract the path and verbs separately (defaults to `GET` when `methods=` is omitted). Handler binding: scan each `.py` file line-by-line; when a matching decorator is found, look ahead for the next non-decorator non-blank line and pair with its `def NAME(` (or `async def NAME(`) via `_PY_DEF_LINE_RE`. `#` line comments are pre-stripped so `# @app.get("/x")` doesn't poison the registry. Wired into both `treesitter._convert` and `scip._parse_scip` alongside the existing Falcon/Fastify/Go branches. Verified on real `fastapi/fastapi` repo: HTTP handlers detected throughout `docs_src/`.
- **Go HTTP entry-point heuristics (`extract/treesitter.py::_collect_go_http_routes`)**: parallel to `_collect_fastify_routes` for TS. Three regexes detect chi (`r.Get("/x", h)`), gin (`r.GET("/x", h)`), chi/gin generic dispatcher (`r.Method("PUT", "/x", h)` / `r.Handle("POST", ...)`), and stdlib `net/http` (`http.HandleFunc("/x", h)`, `mux.Handle("/x", h)`). Stdlib routes record verb as `"ANY"` since registration doesn't specify one. Returns `dict[bare_handler_name → (METHOD, path)]` — same shape as Fastify, so wiring into both `treesitter._convert` and `scip._parse_scip` is one extra `elif lang == "go"` branch. **Source is pre-stripped of `// line comments`** before matching so commented-out or doc-example registrations don't poison the route registry. Receiver `http` is blocklisted to avoid mistaking the HTTP *client* call `http.Get(url)` for a route registrar.
...
- **Web visualizer (`orgraph/web.py` + `orgraph/web_static/`, CLI `orgraph serve-web`)**: a local, MIT-licensed browser UI over the pre-built `.orgraph/graph.kuzu` — built because GitNexus's equivalent is PolyForm-Noncommercial (unusable for commercial team tooling). **No new dependencies**: stdlib `http.server` (`ThreadingHTTPServer`, `allow_reuse_address=True`) + the existing `query.py`. Endpoints: `/api/graph` (whole symbol graph — nodes with id/name/kind/file/line/community/http_method/indegree, links for CALLS/INHERITS/IMPLEMENTS; soft cap `_MAX_NODES=6000` keeping top-indegree), `/api/node?uid=` (callers/callees/source), `/api/search?q=`, `/api/entrypoints` (queried directly for `uid`, which `query.get_http_handlers` omits), `/api/path?from=&to=`. Frontend is a single `web_static/index.html` + **vendored** `force-graph.min.js` (v1.51.4 UMD — canvas-based, handles thousands of nodes; vendored so it works offline, no CDN/CSP). **Concurrency model**: Kuzu takes an exclusive lock even read-only, so `GraphData` copies the DB to a temp dir ONCE at startup (`_open`) and serves every request from one shared connection serialized under `_lock` (Kuzu connections aren't thread-safe; ThreadingHTTPServer is concurrent). This also lets a concurrent `orgraph index` keep writing the original. **Build note**: `web_static/*` must stay in `pyproject.toml [tool.hatch.build.targets.wheel] include` or hatchling drops the non-`.py` assets from the wheel (verified present in 0.1.34 wheel). **UI design (redesigned 2026-06-22)**: Default view is Entry Points table (not the graph) — a table of all HTTP routes with colored method badges (GET/POST/PUT/DELETE/PATCH), path, handler name, file:line; clicking a row opens the Inspector. Hot Functions view shows top 50 symbols by in-degree with inline bar charts. Files view shows directory tree with symbol counts. Call Graph view is opt-in (force-graph, focuses on node neighborhood when a symbol is selected). Inspector drawer (380px, slides in from right) shows kind badge, source snippet with line numbers, callers list, callees list — each clickable to navigate. Path finder in inspector: type a target symbol name → calls `/api/path` → renders the BFS hop chain. Command palette (/ or Cmd+K): live symbol search with kind badges, keyboard-navigable. Aesthetic: dark-forest monospace (phosphor green `#5cf050` on `#050806`, `ui-monospace` throughout). **Deployment note**: `orgraph serve-web` runs from the globally-installed package; `web_static/` changes must be copied to the installed location (e.g. `~/.local/share/uv/tools/orgraph-mcp/lib/python3.13/site-packages/orgraph/web_static/`) or the package reinstalled to take effect. Verified end-to-end on `backend-tss-api_v2`: 1,893 nodes / 2,760 edges / 310 entry points render in-browser with zero console errors.
...
orgraph serve-web <repo>          # browser visualizer (force graph + sidebar + search); --port 4747
```
